...
# Supports: Debian/Ubuntu (apt), Fedora (dnf), Arch (pacman), FreeBSD (pkg)

echo "=========================================="
echo "   Emap Universal System Setup"
echo "=========================================="

# 0. Check for Root / Sudo
if [ "$EUID" -ne 0 ]; then
    if command -v sudo &> /dev/null; then
        SUDO="sudo"
    else
        echo "Error: This script needs root privileges. Please install 'sudo' or run as root."
        exit 1
    fi
else
    SUDO=""
...
echo "[2/6] Setting up UI dependencies (Node.js)..."
cd ui
...
echo "[3/6] Configuring USB auto-mounting and links..."
$SUDO mkdir -p /media
...
echo "[4/6] Building Emap Application..."
cargo build --release
